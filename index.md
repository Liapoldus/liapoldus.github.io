...
Liapoldus — независимый multi-tenant L7 gateway («микро-nginx») для публичных
сайтов. Он раздаёт статические артефакты из локального registry-каталога,
применяет маршруты и редиректы, обслуживает TLS/HTTP/2 и вызывает внешние
**plugins** — отдельные процессы с формами, капчей и другой логикой.
...
- [Configuration](/configuration/) — полный справочник по `gateway.yaml`,
  management API и безопасности.
- [CLI](/cli/) — подкоманды: версии, откат, диагностика.
- [Deploy](/deploy/) — локальная разработка, Docker Compose, переменные
  окружения.
- [Архитектура](/architecture/) — целевая структура проектов, data/control
  plane gateway, plugin protocol.
- [Плагины](/plugins/) — декларация, жизненный цикл, контракт протокола и
  гайд создания своих плагинов.