...
Matcher может рекурсивно объединять условия через `all`, `any` и `not`.
Обычные поля одного matcher и его композиционные группы объединяются через
AND. Элементы `all` и `any` проверяются слева направо в порядке YAML.
Результат Geo/ASN lookup имеет третье состояние «неизвестно»: в `all` любой
доказанный `false` определяет всю группу как `false`, а в `any` любой
доказанный `true` определяет группу как `true`; в противном случае ошибка
lookup передаётся в `onError` правила. `not` не инвертирует ошибку lookup.
Для пустой группы `all` результат — `true`, для пустой группы `any` — `false`.

В HTTP WAF поле `requestSize` означает фактическое число байт тела запроса
после снятия HTTP transfer framing. Строка запроса и заголовки не учитываются;
chunked body измеряется по реально прочитанным данным, а не по
`Content-Length`. Если policy содержит `requestSize` (включая вложенные
`all`/`any`/`not`), Gateway дочитывает и сохраняет тело до WAF-проверки, затем
передаёт те же байты выбранному upstream или plugin. Чтение ограничено
`listeners.<name>.limits.bodyBytes`; превышение завершается `413 body_too_large`
до вызова upstream или plugin. Порог задаётся целым числом байт либо строкой
с суффиксом `KiB`, `MiB` или `GiB`; сравнения `gt`, `gte`, `lt`, `lte` имеют
обычную строгую/включительную семантику.
