...
| Cookies | Management `GET`/`PUT`, ETag/If-Match CAS, SQLite schema v3 и audit одной транзакцией; embedded Caddy активирует candidate dispatch bindings перед commit и восстанавливает прежние при ошибке. | Нужны child-process conformance для startup reload, реальной request-cookie фильтрации и typed response actions, а также crash-recovery/rollback-failure gate. External Caddy пока возвращает unavailable до private snapshot sync. |
