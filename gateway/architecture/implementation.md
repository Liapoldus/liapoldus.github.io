...
| Cookies | Изолированные Caddy `Call`/`Stream` handler tests покрывают allow-list и typed cookie response actions. `serve` загружает local plugin runtime из SQLite и передаёт bindings embedded Caddy. | Gateway-owned allow-list пока не хранится и не передаётся в dispatch snapshot; сквозной cookie flow через запущенный Gateway не доказан. |
